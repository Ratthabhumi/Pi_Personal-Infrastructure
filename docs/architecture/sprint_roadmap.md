...
- [ ] Running `make bootstrap` from the Dell Windows Workstation securely logs into `mew@homelab` over Tailscale without manual password typing (SSH Keys configured).
- [ ] Ubuntu Firewall (UFW) active, blocking all external unsolicited ports while trusting Tailscale interface (`tailscale0`).
- [ ] Docker Engine running natively and verified with non-root user permissions (`docker ps` works without `sudo`).
- [ ] Server log rotation and automatic unattended upgrades configured for long-term OS stability.
