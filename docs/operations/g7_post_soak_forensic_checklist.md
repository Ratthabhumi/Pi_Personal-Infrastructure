...
- Order semantics: PaperSessionManifest distinguishes simulated paper execution from real broker execution. The G7 safety invariant strictly requires zero real order submissions (manifest `no_real_orders == true`, `simulated_fills_only == true`, and journal `ORDER_SUBMITTED` count == 0). Simulated paper order activity (e.g. `ORDER_INTENT_CREATED`, `FILL_SIMULATED`, and non-zero manifest `total_order_count` / `total_trade_count` generated by the simulated test strategy) represents purely internal engine accounting and does NOT violate `NO_REAL_ORDERS=true`.
